import {error_occur, update_is_unauthenticated} from '../redux/authSlice';
...
export const error = (rowError: string | any) => {
  if (rowError?.code) {
    if (firebaseErrorCodes[rowError.code]) {
      store.dispatch(error_occur(firebaseErrorCodes[rowError.code]));
    } else {
      store.dispatch(error_occur('Something went wrong'));
    }
  } else if (rowError?.data?.success?.message) {
    if (rowError?.data?.success?.message === 'Unauthenticated.') {
      store.dispatch(update_is_unauthenticated());
    }
    store.dispatch(error_occur(rowError?.data?.success?.message));
  } else if (rowError?.data?.error?.message) {
    if (rowError?.data?.error?.message === 'Unauthenticated.') {
      store.dispatch(update_is_unauthenticated());
    }
    store.dispatch(error_occur(rowError?.data?.error?.message));
  } else if (rowError?.data?.error) {
    if (rowError?.data?.error === 'Unauthenticated.') {
      store.dispatch(update_is_unauthenticated());
    }
    store.dispatch(error_occur(rowError?.data?.error));
  } else if (rowError?.data?.message) {
    if (rowError?.data?.message === 'Unauthenticated.') {
      store.dispatch(update_is_unauthenticated());
    }
    store.dispatch(error_occur(rowError?.data?.message));
  } else if (rowError?.message) {
    if (rowError?.message === 'Unauthenticated.') {
      store.dispatch(update_is_unauthenticated());
    }
    store.dispatch(error_occur(rowError?.message));
  } else if (rowError?.status && httpStatusCodes[rowError.status]) {
    store.dispatch(error_occur(httpStatusCodes[rowError.status]));
  } else {
    store.dispatch(error_occur('Something went wrong'));
  }
};
